...
function updateActiveSlide(slide) {
  ...
  const slides = block.querySelectorAll('.carousel-toolkits-slide');
  ...
  const indicators = block.querySelectorAll('.carousel-toolkits-slide-indicator');
...
export function showSlide(block, slideIndex = 0) {
  ...
  block.querySelector('.carousel-toolkits-slides').scrollTo({
    top: 0,
...
function bindEvents(block) {
  const slideIndicators = block.querySelector('.carousel-toolkits-slide-indicators');
  ...
  block.querySelectorAll('.carousel-toolkits-slide').forEach((slide) => {
...
export default async function decorate(block) {
  ...
  container.classList.add('carousel-toolkits-slides-container');

  const slidesWrapper = document.createElement('ul');
  slidesWrapper.classList.add('carousel-toolkits-slides');
  ...
  if (!isSingleSlide) {
    ...
    slideIndicators.classList.add('carousel-toolkits-slide-indicators');
    slideIndicatorsNav.append(slideIndicators);
    block.append(slideIndicatorsNav);

    const slideNavButtons = document.createElement('div');
    slideNavButtons.classList.add('carousel-toolkits-navigation-buttons');
  ...
  rows.forEach((row, idx) => {
    ...
    if (slideIndicators) {
      ...
      indicator.classList.add('carousel-toolkits-slide-indicator');
